...
export function registerController(name, ControllerClass) {
    ...
    let elements = querySelectorAll('[data-'+ ENV.HTMLDATA().CONTROLLER_DEF +'="'+name+'"]');
    ...
    for(let i=0, max=elements.length; i<max; i++) {
        ...
        console.log(ENV.HTMLDATA().CONTROLLER_REF);
    ...
    while(i--) {
        ...
        const observerId = addObserver(controller, keyName, function(keyName, element) {
            ...
            setTimeout(function() {
                ...
                let elements = querySelectorAll(element, '[data-' + ENV.HTMLDATA().ACTION + ']');
